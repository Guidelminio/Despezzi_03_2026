...
export default function Login() {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      if (!authWindow) {
        toast.error('Por favor, permita popups para fazer login com o Google.');
      }
    } catch (error) {
    ...
    <div className="min-h-screen flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-surface-light dark:bg-surface-dark rounded-2xl p-8 shadow-lg border border-slate-200 dark:border-slate-800">
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Acesse sua conta para gerenciar suas finanças.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <div className="flex justify-between items-center mb-1">
              ...
              <Link to="/forgot-password" className="text-xs text-primary font-medium hover:underline">Esqueceu a senha?</Link>
            </div>
            <input
              type="password"
              required
        ...
        <div className="mt-6 flex items-center justify-center gap-4">
          ...
          <span className="text-xs text-slate-500 font-medium uppercase">Ou continue com</span>
          <div className="h-px bg-slate-200 dark:bg-slate-800 flex-1"></div>
        </div>
        ...
        <button 
          ...
          type="button" 
          className="mt-6 w-full flex items-center justify-center gap-3 py-3 rounded-lg border border-slate-300 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors font-medium"
        >
